...
test("VerificationService stores register code in Redis for five minutes", async () => {
  ...
  const result = await service.sendCode("User@Example.com");

  assert.deepEqual(result, { success: true, remainingSeconds: 60 });
  assert.equal(redisClient.records.get("test:code:user@example.com").ttlSeconds, 300);
  assert.equal(redisClient.records.get("test:cooldown:user@example.com").ttlSeconds, 60);
